...
class DoomMain<T, V> : DoomStatus<T, V>(), IDoomGameNetworking, IDoomGame, IDoom {
    ...
    protected fun DoLoadGame() {
        try {
            ...
            run { // P_UnArchivePlayers P_UnArchiveWorld P_UnArchiveThinkers P_UnArchiveSpecials
                ok = dsg.doLoad(f)
            }
            f.close()
    ...
    protected fun DoSaveGame() {
        try {
            ...
            DataOutputStream(FileOutputStream(name)).use { f ->
                run { // P_ArchivePlayers P_ArchiveWorld P_ArchiveThinkers P_ArchiveSpecials
                    val ok = dsg.doSave(f)
                }
            }
        } catch (e: IOException) {
